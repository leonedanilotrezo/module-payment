define([
	...
	var _eventNormalize = function (listener) {
		return function (e) {
			var newEvt;
			if (e == null) {
				e = window.event;
	...
	var _on = function (ele, event, listener) {
	...
	var _off = function (ele, event, listener) {
	...
	var lengthFromType = function (type) {
		let card = cardFromType(type);
		return (card != 'undefined' ? card.length : null);
	};

	var getDirectionality = function (target) {
	...
	var replaceFullWidthChars = function (str) {
		let c, chars, fullWidth, halfWidth, i, idx, len, value;
		if (str == null) {
			str = '';
		}
	...
	var formatCardExpiry = function (e) {
		var digit, val;
		digit = String.fromCharCode(e.which);
		if (!/^\d+$/.test(digit)) {
			return;
	...
	var formatForwardSlashAndSpace = function (e) {
		var val, which;
	...
	var reFormatCardNumber = function(e) {
		var cursor = _getCaretPos(e.target);
		if (e.target.value === '') {
			return;
		}
	...
	var attachEvents = function (input, events, detach) {
		var evt, i, len;
	...
	class PayForm {
		...
		lengthFromType (cardType) {
			return lengthFromType(cardType);
		}
